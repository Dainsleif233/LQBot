...
- **引用事件的 `ref_msg_idx` 不能当作反查机器人消息的唯一凭据（实测）**：同一条机器人消息，用户引用时平台可能给出与发送响应 `ext_info.ref_idx` 不同的索引（两者只差开头 22 字符的消息段、会话尾段相同）；引用转发副本/并行消息（message_type 101）时它指向「包装节点」，官方群事件示例里甚至出现过 `TMP_<uuid>`。框架因此不再只读一个字段：`parseQuote` 把 `msg_elements`（含嵌套）与 `parallel_message.msg_nodes` 的被引用正文递归收进 `quote.elementTexts`，把元素作者汇总成 `quote.botAuthored`（取 `MsgElement.author.bot`：true=机器人 / false=用户 / null=事件未带 author，QQ 常省略），`dispatchQuote` 的放行条件放宽为「有 `ref_msg_idx` **或** 有被引用正文」。**插件约定**：需要「引用作答 / 引用交互」的命令，应把自证标识（会话 id、题号等）写进自己发出的消息文本里，引用时用 `ctx.quote.elementTexts` 取回并自行反查，同时用 `ctx.quote.botAuthored` 拒绝「用户把标识抄进自己的消息再引用」；范式见 `src/commands/question.ts`（题号写首行 + 作者校验）。
